...
import { AvailableNetworks, ComplianceInfo, KnownCurrencies, Networks } from "./types";

import { BigNumber, Contract, ethers, utils } from 'ethers'
import { getTornadoEventsDb, initTornadoEventsDB, isInitialized, TornadoEvents, updateTornadoEvents } from './events';
import config from './config';
import MixerAbi from './abi/Mixer.abi.json';

// Set provider
const providers: {[network in AvailableNetworks]: ethers.providers.JsonRpcProvider | undefined} = {
    mainnet: undefined,
    goerli: undefined
}
...
export const isValidNoteString = (noteString: string) => {
    const noteRegex = /blank-(?<currency>\w+)-(?<amount>[\d.]+)-(?<chainId>\d+)-0x(?<note>[0-9a-fA-F]{124})/g
...
export const getComplianceInformation = async (
    ...
    const contractAddress: string = (deployments as any)[networkKey][pair.currency].instanceAddress[pair.amount]
    const contract = new Contract(contractAddress, MixerAbi, getProvider(network as AvailableNetworks))
    ...
    const spent = await getTornadoEventsDb()!.isSpent(network as AvailableNetworks, pair, parsedDeposit.nullifierHex)

    // Get transaction receipt
    const receipt = await getProvider(network as AvailableNetworks)
    ...
    const withdrawEv =
      await getTornadoEventsDb()!.getWithdrawalEventByNullifier(
        network as AvailableNetworks,
        pair,
